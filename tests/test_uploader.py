...
from ddw.uploader import download, head_object, md5_content, should_upload_new, upload
...
class TestDownload:
    def test_returns_object_body_as_string(self, client_mock):
        client_mock.get_object.return_value = {"Body": BytesIO(b"<html></html>")}

        assert download("bucket", "key") == "<html></html>"
        client_mock.get_object.assert_called_once_with(Bucket="bucket", Key="key")

    def test_when_client_error_raised_returns_none(self, client_mock):
        client_mock.get_object.side_effect = ClientError({}, "bar")

        assert download("bucket", "key") is None
...
class TestUpload:
    ...
    def test_when_forced_upload_skips_change_check(
        self, client_mock, should_upload_mock
    ):
        upload("food", "bar", "gaz", force=True)

        should_upload_mock.assert_not_called()
        client_mock.put_object.assert_called_once_with(
            Bucket="bar",
            Key="gaz",
            Body="food",
